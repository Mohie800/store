...
export default function CardList(props) {
    ...
    const sum1 = props.products.map(pro => {
  ...
  return (
    <Transition.Root show={props.open} as={Fragment} >
      <Dialog as="div" className="fixed inset-0 overflow-hidden" onClose={props.setOpen}>
        <div className="absolute inset-0 overflow-hidden" id='card-list' style={{zIndex:"-1"}}>
          <Transition.Child
            as={Fragment}
                            ...
                            <li key={product.id} className="flex py-6">
                              ...
                              <div className="ml-4 flex flex-1 flex-col">
                                ...
                                <div className="flex flex-1 items-end justify-between text-sm">
                                  <p className="text-gray-500">Qty {product.pAmount}</p>

                                  <div className="flex">
                                    <button onClick={()=>props.remove(product)} type="button" className="font-medium text-indigo-600 hover:text-indigo-500">
                                      Remove
                                    </button>
                                  </div>
                                </div>
