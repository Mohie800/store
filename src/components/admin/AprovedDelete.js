...
const AprovedDelete = (props) => {
    ...
    const actions = (
        <div>
            <button onClick={()=> props.deleteAproved(props.id())} className="ui negative button">Delete</button>
            <Link to={`/admin/aproved/${props.id()}`} className="ui button">Cancel</Link>
        </div>
    )
...
export default connect(mapStateToProps, { getAproved,deleteAproved })(AprovedDelete);